...
namespace ego_planner
{
  ...
  void BsplineOptimizer::setParam(ros::NodeHandle &nh)
  {
    ...
    nh.param("optimization/lambda_formation", lambda6_, 1.0);
    ...
    nh.param("optimization/desired_dist", desired_dist_, 2.0);
    nh.param("optimization/tolerance_dist", tolerance_dist_, 1.0);
    nh.param("optimization/tolerance_theta", tolerance_theta_, 0.2);

    max_vel_vec_ << max_vel_, max_vel_, 0;
    max_acc_vec_ << max_acc_, max_acc_, 0;
  }
  ...
  void BsplineOptimizer::setMasterTraj(OneTrajDataOfSwarm *traj_ptr) {master_traj_ = traj_ptr; }
  void BsplineOptimizer::setDirection(Eigen::Vector3d dir) {formation_dir_ = dir;}
  void BsplineOptimizer::setFormation(Eigen::Vector3d pos) {formation_pos_ = pos;}
  ...
  {
    ROS_WARN("segments=%d",segments.size());
    ...
    // cout<<"CTRL_PT_DIST"<<CTRL_PT_DIST<<endl;
    ...
    {
      std::vector<ControlPoints> oneSeg;
      ROS_WARN("erase operation make 0 segment");
      oneSeg.push_back(cps_);
      return oneSeg;
    }
    ...
    // cout<<"seg_upbound"<<seg_upbound<<endl;
    for (int i = 0; i < max_traj_nums; i++)
    {
      ...
      while (digit_id < seg_upbound && selection[digit_id] >= VARIS)
      {
        ...
        digit_id++;
        cout<<"digit"<<digit_id<<" "<<selection[digit_id]<<endl;
      ...
      //输出全部元素
      vector<int>::iterator ite = selection.begin();
 
      // cout<<"selection";
      // for (; ite != selection.end(); ite++){
      //   cout << *ite<<" ";
      // }
      // cout << endl;
  ...
  std::vector<std::pair<int, int>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
  {
    ...
    if (flag_first_init)
    {
      ...
      cps_xy_.resize(2,init_points.cols());
    }
    ...
    for (size_t i = 0; i < segment_ids.size(); i++)
    {
      ...
      if (got_intersection_id >= 0)
      {
        for (int j = got_intersection_id + 1; j <= adjusted_segment_ids[i].second; ++j)
          if (!cps_.flag_temp[j])
          {
            cps_.base_point[j].push_back(cps_.base_point[j - 1].back());//第j个控制点对应的牵引点与向量
  ...
  void BsplineOptimizer::calcFormationCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    cost = 0.0;
    int end_idx = q.cols() - order_ - 1; 
    double t_now = ros::Time::now().toSec();
    if(master_traj_ == NULL)
    {
      return;
    }
    for (int i = order_; i < end_idx; i++)
    {
      ...
      // double glb_time =  t_now + (i+order_) * bspline_interval_;
      double traj_i_sart_time = master_traj_->start_time_.toSec();
      if (glb_time < traj_i_sart_time + master_traj_->duration_ - 0.1)
      {
        Eigen::Vector3d master_prid = master_traj_->position_traj_.evaluateDeBoorT(glb_time - traj_i_sart_time);
        Eigen::Vector3d tar_p;
        tar_p.x() = (formation_dir_(0) * (formation_pos_(0)) - formation_dir_(1) * formation_pos_(1)) + master_prid(0);
        tar_p.y() = (formation_dir_(1) * (formation_pos_(0)) + formation_dir_(0) * formation_pos_(1)) + master_prid(1);
      // tar_p.z() = formation_dir_(2) * l + formation_(2, drone_id_) + O(2);
        Eigen::Vector3d q_1, q_2, q_3;
        Eigen::Vector2d dpos;
        q_1 = q.col(i);
        q_2 = q.col(i+1);
        q_3 = q.col(i+2);
        dpos = (1 / 6.0 * (q_3 + 4 * q_2 + q_1) - tar_p).block<2,1>(0,0);
        cost += dpos.squaredNorm();

        // ROS_WARN("formation cost %f",cost);
        gradient.col(i) += 2 * dpos * (1 / 6.0);
        gradient.col(i+1) += 2 * dpos * (4 / 6.0);
        gradient.col(i+2) += 2 * dpos * (1 / 6.0);
        // cout<<"formation grad"<<gradient<<endl;
      }
      else
      {
        // ROS_WARN("glbtime is too new!");
      }
    }
  }
  ...
  {
    cost = 0.0;
    int end_idx = q.cols() - order_ - (double)(q.cols() - 2 * order_) * 2.0 / 3.0; // Only check the first 2/3 points
    ...
    for (int i = order_; i < end_idx; i++)
    {
      ...
      for (size_t id = 0; id < swarm_trajs_->size(); id++)
      {
        ...
        if (glb_time < traj_i_satrt_time + swarm_trajs_->at(id).duration_ - 0.1)
        {
          ...
          Eigen::Vector2d dist_grad = (cps_.points.col(i) - swarm_prid).block<2,1>(0,0);
                                                 ...
                                                 Eigen::MatrixXd &gradient, int iter_num, double smoothness_cost)
  {
    ...
    for (auto i = order_; i < end_idx; ++i)
    {
      ...
      {
        cps_.direction[i][j][2] = 0;
        cps_.direction[i][j].normalize();
        
        double dist = (cps_.points.col(i) - cps_.base_point[i][j]).dot(cps_.direction[i][j]);
        double dist_err = cps_.clearance - dist;
        Eigen::Vector2d dist_grad = cps_.direction[i][j].block<2,1>(0,0);
                                            ...
                                            Eigen::MatrixXd &gradient, bool falg_use_jerk /* = true*/)
  {
    ...
    {
      Eigen::Vector3d jerk;
      Eigen::Vector2d temp_j;
      for (int i = 0; i < q.cols() - 3; i++)
      {
        /* evaluate jerk */
        jerk = q.col(i + 3) - 3 * q.col(i + 2) + 3 * q.col(i + 1) - q.col(i);
        
        temp_j = jerk.block<2,1>(0,0);
        cost += temp_j.squaredNorm();
        temp_j = 2.0 * temp_j;
        /* jerk gradient */
    ...
    else
    {
      ...
      for (int i = 0; i < q.cols() - 2; i++)
      {
        ...
        temp_acc = acc.block<2,1>(0,0);
        cost += temp_acc.squaredNorm();
        temp_acc = 2.0 * temp_acc;
        /* acc gradient */
    ...
    //  ROS_WARN("smoothness cost %f",cost);
  }

  void BsplineOptimizer::calcVisibilityCost(const Eigen::MatrixXd &q, double &cost,
                                            Eigen::MatrixXd &gradient)
  {
    cost = 0.0;
    int end_idx = q.cols() - order_ - (double)(q.cols() - 2 * order_) * 1.0 / 3.0;; 
    double t_now = ros::Time::now().toSec();
    double glb_time = t_now + ((double)(order_ - 1) / 2 + 1) * bspline_interval_;
    if(target_ps.size() == 0)
    {
      return;
    }
    int region_id = 0;
    double test_time=visible_T;

    for (int i = 0 ; i < visible_ps.size(); i++)
    {
      if(glb_time >= test_time && glb_time < (test_time+bspline_interval_))
      {
        if(glb_time - test_time > (test_time+bspline_interval_) -glb_time)
        {
           region_id++;   
        }
        // cout <<"region_id:"<< region_id<<endl;
        // cout <<"difference:"<<glb_time - visible_T<<endl;//0.4ms的级别
        break;
      }
      else if(glb_time<test_time)
      {
          ROS_ERROR("what problem???");
      }
      test_time += bspline_interval_;
      region_id++;
    }
    for (int i = order_; i < end_idx; i++)
    {
      if(region_id >= visible_ps.size())
      {
        break;
      }
      else 
      {
        Eigen::Vector2d a = (cps_.points.col(i)  - target_ps[region_id]).block<2,1>(0,0);
        Eigen::Vector2d b = (visible_ps[region_id] - target_ps[region_id]).block<2,1>(0,0);
        double inner_product = a.dot(b);
        double norm_a = a.norm();
        double norm_b = b.norm();
        double theta_less = thetas[region_id] - tolerance_theta_ > 0 ? thetas[region_id]  - tolerance_theta_ : 0;
        double cosTheta = cos(theta_less);
        double pen = cosTheta - inner_product / norm_a / norm_b;
        if (pen > 0) 
        {
          cost += pen*pen*pen;
          gradient.col(i) += - 3*pen*pen * (norm_a * b - inner_product / norm_a * a) / norm_a / norm_a / norm_b;
          // gradp = grad * (norm_b * cosTheta / norm_a * a - b);
        }

        if(norm_a > desired_dist_ + tolerance_dist_)
        {
          double delta_dis = norm_a - desired_dist_ - tolerance_dist_;
          cost += delta_dis*delta_dis;
          gradient.col(i) += 2*delta_dis*a/norm_a;
        }
        else if(norm_a < desired_dist_ - tolerance_dist_)
        {
          double delta_dis =  desired_dist_ - tolerance_dist_ - norm_a;
          cost += delta_dis*delta_dis;
          gradient.col(i) += -2*delta_dis*a/norm_a;
        }
      }
  ...
  void BsplineOptimizer::calcTerminalCost(const Eigen::MatrixXd &q, double &cost, Eigen::MatrixXd &gradient)
  {
    ...
    Eigen::Vector3d q_3, q_2, q_1;
    Eigen::Vector2d temp_grad;
    q_3 = q.col(q.cols() - 3);
    q_2 = q.col(q.cols() - 2);
    q_1 = q.col(q.cols() - 1);

    temp_grad = (1 / 6.0 * (q_3 + 4 * q_2 + q_1) - local_target_pt_).block<2,1>(0,0);
    cost += temp_grad.squaredNorm();

    gradient.col(q.cols() - 3) += 2 * temp_grad * (1 / 6.0);
    gradient.col(q.cols() - 2) += 2 * temp_grad * (4 / 6.0);
    gradient.col(q.cols() - 1) += 2 * temp_grad * (1 / 6.0);
  }
                                             ...
                                             Eigen::MatrixXd &gradient)
  {
    ...
    for (int i = 0; i < q.cols() - 1; i++)
    {
      ...
      for (int j = 0; j < 2; j++)
      {
        if (vi(j) > max_vel_vec_(j))
        {
          // cout << "zx-todo VEL" << endl;
          // cout << vi(j) << endl;
          cost += pow(vi(j) - max_vel_vec_(j), 2) * ts_inv2; // multiply ts_inv3 to make vel and acc has similar magnitude

          gradient(j, i + 0) += -2 * (vi(j) - max_vel_vec_(j)) / ts * ts_inv2;
          gradient(j, i + 1) += 2 * (vi(j) - max_vel_vec_(j)) / ts * ts_inv2;
        }
        else if (vi(j) < -max_vel_vec_(j))
        {
          cost += pow(vi(j) + max_vel_vec_(j), 2) * ts_inv2;
    ...
    for (int i = 0; i < q.cols() - 2; i++)
    {
      ...
      for (int j = 0; j < 2; j++)
      {
        if (ai(j) > max_acc_vec_(j))
        {
          // cout << "zx-todo ACC" << endl;
          // cout << ai(j) << endl;
          cost += pow(ai(j) - max_acc_vec_(j), 2);

          gradient(j, i + 0) += 2 * (ai(j) - max_acc_vec_(j)) * ts_inv2;
          gradient(j, i + 1) += -4 * (ai(j) - max_acc_vec_(j)) * ts_inv2;
          gradient(j, i + 2) += 2 * (ai(j) - max_acc_vec_(j)) * ts_inv2;
        }
        else if (ai(j) < -max_acc_vec_(j))
        {
          cost += pow(ai(j) + max_acc_vec_(j), 2);
  ...
  bool BsplineOptimizer::BsplineOptimizeTrajRebound(Eigen::MatrixXd &optimal_points, double ts)
  {
    ...
    double final_cost;
    cps_xy_ = cps_.points.block(0,0,2,cps_.size);
  ...
  bool BsplineOptimizer::rebound_optimize(double &final_cost)
  {
    ...
    do
    {
      ...
      memcpy(q, cps_xy_.data() + 2 * start_id, variable_num_ * sizeof(q[0]));//Eigen默认是按列顺序存储
...
//#define USE_SECOND_CLEARENCE_CHECK
  ...
  void BsplineOptimizer::combineCostRebound(const double *x, double *grad, double &f_combine, const int n)
  {
    ...
    memcpy(cps_xy_.data() + 2 * order_, x, n * sizeof(x[0]));

    cps_.points.block(0,0,2,cps_.size) = cps_xy_;
    ...
    // Eigen::MatrixXd g_limitz = Eigen::MatrixXd::Zero(2, cps_.size);
    Eigen::MatrixXd g_formation = Eigen::MatrixXd::Zero(2, cps_.size);
    Eigen::MatrixXd g_visibility = Eigen::MatrixXd::Zero(2, cps_.size);
    ...
    // calcFormationCost(cps_.points, f_formation, g_formation);
    calcVisibilityCost(cps_.points, f_visibility, g_visibility);

    
    // cout<<fixed << setprecision(6)<<"control points\n"<<cps_.points<<endl;
    ...
    f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + new_lambda2_ * f_swarm + lambda2_ * f_terminal + lambda6_  * f_visibility;
    //f_combine = lambda1_ * f_smoothness + new_lambda2_ * f_distance + lambda3_ * f_feasibility + new_lambda2_ * f_mov_objs;
    //printf("origin %f %f %f %f\n", f_smoothness, f_distance, f_feasibility, f_combine);

    Eigen::MatrixXd grad_2D = lambda1_ * g_smoothness + new_lambda2_ * g_distance + lambda3_ * g_feasibility + new_lambda2_ * g_swarm + lambda2_ * g_terminal  + lambda6_  * g_visibility;
...
//在前进方向上向后生成扇形
  void BsplineOptimizer::generateVisbilityRegion(const double &ts, const Eigen::Vector3d &start_pt)
  {
    if(master_traj_ == NULL)
    {
      return;
    }
    visible_T = ros::Time::now().toSec();
    target_ps.clear();
    visible_ps.clear();
    thetas.clear();
    Eigen::Vector3d target_p, visible_p, seed_p, dir;
    double r = desired_dist_+tolerance_dist_;
    // double theta0 =  atan2(-formation_dir_.y(), -formation_dir_.x());
    double theta0;
    double d_theta = grid_map_->getResolution() /  r / 2;
  
    // double glb_time = visible_T + ((double)(order_ - 1) / 2 + 1) * ts;
    // visible_T = glb_time;
    double glb_time = visible_T ;
    
    double traj_sart_time = master_traj_->start_time_.toSec();
    double end_time = traj_sart_time + (master_traj_->duration_ *3/4);
    while(glb_time < end_time)
    {
      // cout<<"in loop "<<endl;
      target_p = master_traj_->position_traj_.evaluateDeBoorT(glb_time - traj_sart_time);

      dir =  master_traj_->position_traj_.getDerivative().evaluateDeBoorT(glb_time - traj_sart_time);
      theta0 =  atan2(-dir.y(), -dir.x());
      target_p.z() = start_pt.z();
      seed_p = target_p - dir*  r;

      //TODO ： 在+-PI上生成多个扇形
      double t_l, t_r, theta;
      for (t_l = theta0 - d_theta; t_l > theta0 - M_PI/2; t_l -= d_theta) 
      {
        Eigen::Vector3d p = target_p;
        p.x() +=  r * cos(t_l);
        p.y() +=  r * sin(t_l);
        // cout<<"check -  "<<endl;//target_p.transpose()<<" "<<p.transpose()<<endl;
        if (!checkRayValid(target_p,p)) {
          t_l += d_theta;
          break;
        }
      }
      for (t_r = theta0 + d_theta; t_r < theta0 + M_PI/2; t_r += d_theta) 
      {
        // cout<<"check +  "<<endl;
        Eigen::Vector3d p = target_p;
        p.x() +=  r * cos(t_r);
      ...
      theta = (t_r - t_l) / 2;
      visible_p = target_p;
      visible_p.x() +=  r * cos(theta_v);
      visible_p.y() +=  r * sin(theta_v);

      target_ps.push_back(target_p);
  ...
  inline bool BsplineOptimizer::checkRayValid(const Eigen::Vector3d& p0, const Eigen::Vector3d& p1) 
  {
    Eigen::Vector3d dp = p1 - p0;
    Eigen::Vector3i idx0,  idx1;
    double res = grid_map_->getResolution();
    idx0 = (p0 / res).array().floor().cast<int>();
    idx1 = (p1 / res).array().floor().cast<int>();
    
    // cout<<"idx- "<<idx0.transpose()<<" "<< idx1.transpose()<<endl;
    // cout<<idx0.array()<<endl;
    ...
    for (int i = 0; i < 3; ++i) 
    {
      t_max(i) = step(i) > 0 ? (idx0(i) + 1) - p0(i) / res : p0(i) / res - idx0(i);
    }
    t_max = t_max.cwiseProduct(delta_t);
    Eigen::Vector3i rayIdx = idx0;
    while ((rayIdx - idx1).squaredNorm() > 1) 
    {
      Eigen::Vector3d tmp = Eigen::Vector3d(rayIdx(0), rayIdx(1), rayIdx(2));
      tmp += half;
      tmp *= res;
      if (grid_map_->getInflateOccupancy(tmp)) {
        return false;
      }
