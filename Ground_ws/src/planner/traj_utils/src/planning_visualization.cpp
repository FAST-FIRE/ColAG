...
namespace ego_planner
{
  PlanningVisualization::PlanningVisualization(ros::NodeHandle &nh)
  {
    ...
    visible_region_pub = nh.advertise<visualization_msgs::Marker>("visible_region", 10);
  }
  ...
  void PlanningVisualization::visualize_fan_shape_meshes(const std::vector<Eigen::Vector3d>& v0,
                                  ...
                                  const std::vector<double>& thetas) 
{
    ...
    setMarkerColor(marker, Color::green, 0.1);
    int M = v0.size();
    // int M = 1;
    cout<<"visible size"<<v0.size()<<endl;
    for (int i = 0; i < M; ++i) 
    {
      //  cout<<"v0 "<<v0[i].transpose()<<"v1 "<<v1[i].transpose()<<"theta "<<thetas[i]<<endl;
    ...
    visible_region_pub.publish(marker);
  }
