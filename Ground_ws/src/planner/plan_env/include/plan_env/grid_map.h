...
struct MappingParameters
{
  ...
  double obstacles_inflation_,obstacles_inflation_unknow_, obstacles_inflation_z_;
  string frame_id_;
  int pose_type_;
...
struct MappingData
{
  ...
  std::vector<char> occupancy_buffer_unknowinflate_;
  ...
  vector<bool> point_record_;
  vector<char> count_inflate_;
  ...
  Eigen::Vector3i global_bound_min_, global_bound_max_;
  Eigen::Vector3d update_bound_min_, update_bound_max_;
  // computation time
...
public:
  ...
  inline bool isInUpdateBox(const Eigen::Vector3d &pos);
  inline void toIndex(int &addr, Eigen::Vector3i &id);
  ...
  inline int getUnknownInflateOccupancy(Eigen::Vector3d pos);
...
private:
  ...
  void masterOdomCallback(const nav_msgs::OdometryConstPtr &odom);
  void otherOdomCallback(const nav_msgs::OdometryConstPtr &odom);
  void occuinflateRecvCallback(const custom_msgs::map_infoPtr &img);
  ...
  void updateInflateLocalMap();

  bool synCloudAndOdom();
  void inflateCount(const Eigen::Vector3i &idx, const bool is_up, vector<char>&map, const double size, const double size_z);
  void updateInflateMap(const Eigen::Vector3i &idx, bool occupy_ori, bool occupy_unknown_ori, bool occupy,bool occupy_unknown);
  ...
  ros::Subscriber indep_cloud_sub_, indep_odom_sub_, extrinsic_sub_, occ_inflate_sub_, master_odom_sub_, other_odom_sub_;
  ...
  double selfbox_x, selfbox_y, selfbox_z;
  int self_id;
  std::vector<pair<double, Eigen::Vector3d>> other_pos_buf_;
};
...
inline void GridMap::toIndex(int &addr, Eigen::Vector3i &id)
{
  id(0) = addr/(mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2));
  id(1) = (addr - id(0)*mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2))/mp_.map_voxel_num_(2);
  id(2) = addr - id(0)*mp_.map_voxel_num_(1) * mp_.map_voxel_num_(2) - id(1)*mp_.map_voxel_num_(2);
}
...
inline int GridMap::getUnknownInflateOccupancy(Eigen::Vector3d pos)
{
  if (!isInMap(pos))
    return -1;
  ...
  return int(md_.occupancy_buffer_unknowinflate_[toAddress(id)] >= 1);
}
...
inline bool GridMap::isInUpdateBox(const Eigen::Vector3d &pos)
{
  if (pos(0) < md_.update_bound_min_(0) + 1e-4 || pos(1) < md_.update_bound_min_(1) + 1e-4 ||
      pos(2) < md_.update_bound_min_(2) + 1e-4)
  {
    return false;
  }
  if (pos(0) > md_.update_bound_max_(0) - 1e-4 || pos(1) > md_.update_bound_max_(1) - 1e-4 ||
      pos(2) > md_.update_bound_max_(2) - 1e-4)
  {
    return false;
  }
  return true;
