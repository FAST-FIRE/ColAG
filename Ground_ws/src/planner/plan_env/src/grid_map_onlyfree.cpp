...
void GridMap::initMap(ros::NodeHandle &nh)
{
  ...
  md_.occupancy_buffer_ = vector<double>(buffer_size, mp_.min_occupancy_log_ + 0.1);
  ...
  md_.occupancy_buffer_inflate_ = vector<char>(buffer_size, inf_num);
...
void GridMap::occuinflateRecvCallback(const sensor_msgs::PointCloud2ConstPtr &msg)
{
  ...
  pcl::PointXYZ pt;
  Eigen::Vector3d p3d;
  Eigen::Vector3i index, index_inf;
  double max_x, max_y, max_z, min_x, min_y, min_z;
  int inf_step = ceil(mp_.obstacles_inflation_ / mp_.resolution_);
  int inf_step_z = ceil(mp_.obstacles_inflation_z_ / mp_.resolution_);

  pt = latest_cloud.points[0];
  p3d(0) = pt.x, p3d(1) = pt.y, p3d(2) = pt.z;
  // cout<< "map_center " <<p3d.transpose()<< "size" << latest_cloud.points.size() <<endl;
  max_x = p3d(0) + mp_.local_update_range_(0);
  max_y = p3d(1) + mp_.local_update_range_(1);
  max_z = p3d(2) + mp_.local_update_range_(2);

  min_x = p3d(0) - mp_.local_update_range_(0);
  min_y = p3d(1) - mp_.local_update_range_(1);
  min_z = p3d(2) - mp_.local_update_range_(2);
  ...
  if (map_way == 1)
  {
    for (size_t i = 1; i < latest_cloud.points.size(); ++i)
    {
      pt = latest_cloud.points[i];
      p3d(0) = pt.x, p3d(1) = pt.y, p3d(2) = pt.z;

      // cout << p3d.transpose() << endl;
      posToIndex(p3d, index); // index is the 3d index of x,y,z//address is the index of one demension
      int addr = toAddress(index);
      md_.point_record_[addr] = true;
      if (md_.occupancy_buffer_[addr] >= mp_.min_occupancy_log_)
      {
        for (int x = -inf_step; x <= inf_step; ++x)
          for (int y = -inf_step; y <= inf_step; ++y)
            for (int z = -inf_step_z / 3; z <= inf_step_z; ++z)
            {
              index_inf(0) = index(0) + x;
              index_inf(1) = index(1) + y;
              index_inf(2) = index(2) + z;
    ...
    md_.global_bound_max_(2) = max(md_.global_bound_max_(2), md_.local_bound_max_(2));
    // cout<<"test"<<endl;
    // cout<<md_.global_bound_min_<<endl;
    // cout<<md_.global_bound_max_<<endl;

    for (int pos_x = md_.local_bound_min_(0); pos_x <= md_.local_bound_max_(0); ++pos_x)
      for (int pos_y = md_.local_bound_min_(1); pos_y <= md_.local_bound_max_(1); ++pos_y)
        for (int pos_z = md_.local_bound_min_(2); pos_z <= md_.local_bound_max_(2); ++pos_z)
        {
...
void GridMap::publishMapInflate(bool all_info)
{
  ...
  for (int x = min_cut(0); x <= max_cut(0); ++x)
    for (int y = min_cut(1); y <= max_cut(1); ++y)
      for (int z = min_cut(2); z <= max_cut(2); ++z)
      {
        if (md_.occupancy_buffer_inflate_[toAddress(x, y, z)] <= 0) // occupancy_buffer_inflate_
          continue;
...
//   int inf_step = ceil(mp_.obstacles_inflation_ / mp_.resolution_);
//   int inf_step_z = ceil(1.0 / mp_.resolution_);//1
// //a loop with point to get the local bound and mark the point that have updated
...
//     int idx_add = toAddress(index);

//     Eigen::Vector3d devi = Eigen::Vector3d(pt.x, pt.y, pt.z)- md_.camera_pos_;
//     if(fabs(devi(0)) <= selfbox_x && fabs(devi(1)) <= selfbox_y && fabs(devi(2)) <= selfbox_z)
//     {
//       md_.occupancy_buffer_[idx_add] = std::min(std::max(md_.occupancy_buffer_[idx_add] +  mp_.prob_miss_log_ , mp_.clamp_min_log_),
//                  mp_.clamp_max_log_);
//     }
//     else
//     {
//       md_.occupancy_buffer_[idx_add] = std::min(std::max(md_.occupancy_buffer_[idx_add] +  mp_.prob_hit_log_ , mp_.clamp_min_log_),
//                  mp_.clamp_max_log_);
//     }

//     md_.point_record_[idx_add] = true;
//   }
//   Eigen::Vector3i min_idx = md_.local_bound_min_;
//   Eigen::Vector3i max_idx = md_.local_bound_max_;
//     //then a loop with local bound will update the free space and inflatemap
//   for (int x = md_.local_bound_min_(0); x <= md_.local_bound_max_(0); ++x)
//     for (int y = md_.local_bound_min_(1); y <= md_.local_bound_max_(1); ++y)
//       for (int z = md_.local_bound_min_(2); z <= md_.local_bound_max_(2); ++z)
//       {
//         int idx_add = toAddress(x,y,z);
//         if(!md_.point_record_[idx_add])
//         {
//           md_.occupancy_buffer_[idx_add] = std::min(std::max(md_.occupancy_buffer_[idx_add] +  mp_.prob_miss_log_ , mp_.clamp_min_log_),
//                 mp_.clamp_max_log_);
//         }
//         else
//         {
//           md_.point_record_[idx_add] = false;
//         }
//         md_.occupancy_buffer_inflate_[idx_add] = 0;
//         //local clear then update;
//       }
//   for (int x = md_.local_bound_min_(0); x <= md_.local_bound_max_(0); ++x)
//     for (int y = md_.local_bound_min_(1); y <= md_.local_bound_max_(1); ++y)
//       for (int z = md_.local_bound_min_(2); z <= md_.local_bound_max_(2); ++z)
...
//             posToIndex(md_.camera_pos_, now_index);
//             /* point inside update range */
//             Eigen::Vector3i devi = Eigen::Vector3i(x,y,z)- now_index;
//             if(devi(2) <= 1 && devi(2) >0)
//             {
