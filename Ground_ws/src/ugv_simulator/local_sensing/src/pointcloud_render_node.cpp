...
int main(int argc, char** argv) {
  ...
  global_map_sub = nh.subscribe("global_map", 1, rcvGlobalPointCloudCallBack, ros::TransportHints().tcpNoDelay());
  local_map_sub = nh.subscribe("local_map", 1, rcvLocalPointCloudCallBack, ros::TransportHints().tcpNoDelay());
  odom_sub = nh.subscribe("odometry", 50, rcvOdometryCallbck, ros::TransportHints().tcpNoDelay());
