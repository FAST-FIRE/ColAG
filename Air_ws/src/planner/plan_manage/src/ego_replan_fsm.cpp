...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(ros::NodeHandle &nh)
  {
    ...
    nh.param("fsm/fly_height", fly_height_, 1.0);
    ...
    {
      waypoint_sub_ = nh.subscribe("waypoints_path", 1, &EGOReplanFSM::pathCallback, this);
      trigger_sub_ =nh.subscribe("/goal", 1, &EGOReplanFSM::waypointCallback, this);
      ROS_INFO("Wait for 1 second.");
      // int count = 0;
      // while (ros::ok() && count++ < 1000)
      // {
  ...
  void EGOReplanFSM::pathCallback(const nav_msgs::PathPtr &msg)
  {
    if(!have_odom_)
      return;

    if(!have_trigger_) have_trigger_ = true;
    ...
    {
      auto pose = *it;
      waypoint << pose.pose.position.x, pose.pose.position.y, fly_height_;
      wps_.push_back(waypoint);
    }
    wp_id_ = 0;

    if(exec_state_ == WAIT_TARGET)
    {
      if((wps_[wp_id_] - odom_pos_).norm() >= 0.2)
        planNextWaypoint(wps_[wp_id_]);
      else
      {
        if(waypoint_num_>1)
    ...
    {
      if((end_pt_ - wps_[wp_id_]).norm() >= 0.20)
      {
        planNextWaypoint(wps_[wp_id_]);
      }
    }
  ...
  void EGOReplanFSM::execFSMCallback(const ros::TimerEvent &e)
  {
    ...
    // cout<<"end_pt"<<end_pt_.transpose()<<endl;
