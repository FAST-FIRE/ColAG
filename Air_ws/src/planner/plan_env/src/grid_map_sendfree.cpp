...
void GridMap::initMap(ros::NodeHandle &nh)
{
  ...
  map_free_pub_ = node_.advertise<sensor_msgs::PointCloud2>("grid_map/occupancy_free", 10);
...
//double dx = pos(0) - md_.ugv_odom_buf_[i].pos.x();
    pt.x = md_.ugv_odom_buf_[i].pos.x();
    pt.y = md_.ugv_odom_buf_[i].pos.y();
    pt.z = md_.ugv_odom_buf_[i].pos.z();
    cloud.push_back(pt);
    Eigen::Vector3i min_bound;
    Eigen::Vector3i max_bound; 
    posToIndex(md_.ugv_odom_buf_[i].pos - mp_.send_update_range_, min_bound);
    posToIndex(md_.ugv_odom_buf_[i].pos + mp_.send_update_range_, max_bound);
    ...
    for (int x = min_bound(0); x <= max_bound(0); ++x)
      for (int y = min_bound(1); y <= max_bound(1); ++y)
        ...
        {
          if (md_.occupancy_buffer_[toAddress(x, y, z)] >= mp_.min_occupancy_log_||
          md_.occupancy_buffer_[toAddress(x, y, z)] < mp_.clamp_min_log_)
          continue;
